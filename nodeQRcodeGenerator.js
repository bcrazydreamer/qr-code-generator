const { createCanvas } = require('canvas')
    ...
    qrcode = function(h,option) {
        var qrSize = 200;
        var qrColor = "#000000";
        var qrBackground = "#ffffff";
        var qrFormat = "image/png";
        if(option){
            if(option.size && typeof(option.size) === 'number'&& (String(option.size).trim().length>0)){
                qrSize = option.size;
            }
            if(option.color && typeof(option.color) === 'string' && (option.color.trim().length>0)){
                qrColor = option.color;
            }
            if(option.background && typeof(option.background) === 'string' && (option.background.trim().length>0)){
                qrBackground = option.background;
            }
            if(option.format && typeof(option.format) === 'string' && (option.format.trim().length>0)){
        ...
        h.typeNumber= -1;
        h.correctLevel= 2;
        h.background= qrBackground;
        h.foreground= qrColor;
        var a;
        a = new o(h.typeNumber, h.correctLevel);
        a.addData(h.text);
        a.make();
        var c = createCanvas(h.width, h.height)
...
/*
  ...
  defualt is also present
*/

x = qrcode("some text",{size:100,format:"png"});
console.log(x);
